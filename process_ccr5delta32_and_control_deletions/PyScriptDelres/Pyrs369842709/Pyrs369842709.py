...
#
    ...
    ref1 = readInt(columns[headerColumns.index("rs16840309_ref")]) #rs75780994
    print(f"ref1: {ref1}")


    N_reads_del = readInt(columns[headerColumns.index("N_reads_del")])
    print(f"N_reads_del: {N_reads_del}")
    alt1 = readInt(columns[headerColumns.index("rs16840309_alt")])  #NOT affected by ancient damage R2 =1
    print(f"alt1: {alt1}")
    ...
    if N_reads_del > 0:
        if N_reads_del >= 2 and N_reads_ref == 0 and (ref1 == 0 or ref1 == -1):  #-1 = nan
            permissive = "DD" #sum_x >= 1 dob allels
        elif N_reads_del >= 1 and N_reads_ref == 0 and ref1 == 0 and alt1 >=1:
            permissive = "DD"

        elif N_reads_del >= 2:
            permissive = "RD"
        elif N_reads_del >= 1 and N_reads_ref !=0  and (ref1 !=0 or alt1 !=0):
            permissive = "RD"
        elif sum_x >=1:  #(N_reads_ref >=1 or ref1 >= 1)
            permissive = "RD"
    ...
    threshold = 4
    if N_reads_del != 0 and N_reads_ref / N_reads_del >= threshold and sum_x == 0:
        permissive = "RR"
        strict = "RR"
        artefactOutFile.write(line)
        
    if N_reads_del == 0 and N_reads_del == 1 and (ref1 == 0 or alt1):
        permissive = "RR"
        strict = "RR"
        artefactOutFile.write(line)
