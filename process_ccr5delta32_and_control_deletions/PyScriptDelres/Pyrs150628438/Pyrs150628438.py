...
#    if columns[0] != 'NEO516':
#        continue
#rs10461016_ref	rs10461016_alt	rs34785754_ref	rs34785754_alt	rs13062383_ref	rs13062383_alt
    ...
    ref1 = readInt(columns[headerColumns.index("rs10461016_ref")])
    print(f"ref1: {ref1}")
    ref2 = readInt(columns[headerColumns.index("rs34785754_ref")])
    print(f"ref2: {ref2}")
    ref3 = readInt(columns[headerColumns.index("rs13062383_ref")])
    print(f"ref3: {ref3}")
    referencecount = readInt(columns[headerColumns.index("referencecount")])
    print(f"referencecount: {referencecount}")

    alt1 = readInt(columns[headerColumns.index("rs10461016_alt")])
    print(f"alt1: {alt1}")
    alt2 = readInt(columns[headerColumns.index("rs34785754_alt")])
    print(f"alt2: {alt2}")
    alt3 = readInt(columns[headerColumns.index("rs13062383_alt")])
    print(f"alt3: {alt3}")
    ...
    if N_reads_del > 0:
        if (N_reads_ref == 0 or N_reads_del >= 3) and referencecount == 0 and haplocount >=1 and referencecount ==0:
            permissive = "DD"
        elif haplocount >= 1:
            permissive = "RD"
        elif N_reads_del >= 2:
            ...
            permissive = "RR"
    elif N_reads_del == 0 and haplocount >=2 and (alt1 >= 1 or alt2 >= 1):
        permissive = "RD"
    else:
        permissive = "RR"
